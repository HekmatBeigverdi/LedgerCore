...
[Authorize]
public class CostCentersController(IUnitOfWork uow) : ControllerBase
{
    ...
    [HttpGet]
    [HasPermission(PermissionCodes.Master_CostCenters_View)]
    public async Task<ActionResult<List<CostCenterDto>>> GetAll(CancellationToken cancellationToken)
    {
        var raw = await uow.Repository<CostCenter>().GetAllAsync(cancellationToken: cancellationToken);
        var items = Unwrap<CostCenter>(raw)
            .OrderBy(x => x.Code)
            .ToList();

        var result = items.Select(x => new CostCenterDto
        {
            Id = x.Id,
            Code = x.Code,
            Name = x.Name,
    ...
    [HttpGet("{id:int}")]
    [HasPermission(PermissionCodes.Master_CostCenters_View)]
    public async Task<ActionResult<CostCenterDto>> Get(int id, CancellationToken cancellationToken)
    {
        var entity = await uow.Repository<CostCenter>().GetByIdAsync(id, cancellationToken);
        if (entity is null)
            return NotFound();
    ...
    [HttpPost]
    [HasPermission(PermissionCodes.Master_CostCenters_Manage)]
    public async Task<ActionResult<CostCenterDto>> Create(
        [FromBody] CostCenterDto request,
        CancellationToken cancellationToken)
    {
        ...
        var repo = uow.Repository<CostCenter>();
        var duplicateCode = await repo.AnyAsync(x => x.Code == request.Code, cancellationToken);
        if (duplicateCode)
            return BadRequest("A cost center with this code already exists.");
    ...
    {
        return BadRequest("Deleting cost centers is not allowed. Deactivate the cost center instead.");
    }
}